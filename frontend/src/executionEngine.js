...
export async function runExecutionPipeline({
  ...
  if (existingFileCount > 0 && !Object.values(active.files || {}).some((f) => f.status !== 'valid')) {
    task('Build', 'done', `${existingFileCount} files already built`);
  } else {
    emit('Building project');
    task('Build', 'running');
    active = await buildAllIncremental(active.project_id, (step, remaining, snapshot) => {
      const total = Object.keys(snapshot.files || {}).length;
      emit(`Building file ${step} (${remaining} remaining of ${total})`);
      task('Build', 'running', `${step} step(s), ${remaining} file(s) remaining`);
    });
    onProject?.(active);
