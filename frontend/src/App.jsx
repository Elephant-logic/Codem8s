...
export default function App() {
  ...
  async function proposeEditFile(text = editInstruction) { if (!projectId || !selectedPath) return null; const original = content; const result = await post(`/projects/${projectId}/files/refactor`, { path: selectedPath, content, instruction: text }); const updated = result.file.content || ''; setPendingEdit({ kind: 'single', files: [{ path: selectedPath, before: original, after: updated, diff: quickDiff(original, updated) }], project: result.project, inspection: result.inspection, instruction: text }); setContent(original); addChat('codem8s', `Prepared changes for ${selectedPath}. Review the diff and approve or reject.`); return result; }
  async function proposeProjectCommand(text) { if (!projectId || !project) return null; const targets = chooseCommandFiles(text, brain, project, selectedPath); if (!targets.length) { addChat('codem8s', 'I could not find safe files to edit for that request. Select a file or ask for a narrower change.'); return null; } let workingProject = project; const changes = []; for (const path of targets) { const before = workingProject.files?.[path]?.content || ''; if (!before) continue; setNotice(`Preparing ${path}...`); const result = await post(`/projects/${workingProject.project_id}/files/refactor`, { path, content: before, instruction: `${text}\n\nThis is part of a coordinated multi-file command. Preserve public APIs unless the request requires changing them.` }); const after = result.file.content || ''; workingProject = result.project; if (after !== before) changes.push({ path, before, after, diff: quickDiff(before, after) }); } if (!changes.length) { addChat('codem8s', 'I checked the likely files but did not produce meaningful changes.'); return null; } setProject(workingProject); setPendingEdit({ kind: 'multi', files: changes, project: workingProject, inspection: null, instruction: text }); addChat('codem8s', `Prepared a multi-file change across ${changes.length} file(s). Review the diffs and approve or reject.`); return workingProject; }
  async function approvePendingEdit(runAfter = false) { if (!pendingEdit) return; setProject(pendingEdit.project); const selectedChange = pendingEdit.files.find((f) => f.path === selectedPath) || pendingEdit.files[0]; if (selectedChange) { setSelectedPath(selectedChange.path); setContent(selectedChange.after); try { setInspection(await request(`/projects/${pendingEdit.project.project_id}/files/${encodeURIComponent(selectedChange.path)}/inspect`)); } catch {} } addChat('system', `Approved AI change to ${pendingEdit.files.length} file(s).`); const id = pendingEdit.project.project_id; setPendingEdit(null); if (runAfter) await runSandbox(id); }
  async function rejectPendingEdit() { if (!pendingEdit) return; let restored = pendingEdit.project; for (const change of pendingEdit.files) restored = await post(`/projects/${restored.project_id}/files/save`, { path: change.path, content: change.before, instruction: 'Rejected AI change restore' }); setProject(restored); const selectedChange = pendingEdit.files.find((f) => f.path === selectedPath) || pendingEdit.files[0]; if (selectedChange) setContent(selectedChange.before); addChat('system', `Rejected AI change and restored ${pendingEdit.files.length} file(s).`); setPendingEdit(null); }
  ...
  async function runUnifiedCommand() {
    ...
    await run('Codem8s is working...', async () => {
      ...
      if (wantsWholeProject(text)) await proposeProjectCommand(text);
      else if (selectedPath) await proposeEditFile(text);
      else await proposeProjectCommand(text);
    });
  }
