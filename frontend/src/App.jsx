...
function wantsWholeProject(text) { return /\b(all files|whole project|entire project|everywhere|across the project|multi[- ]file|app wide|global)\b/i.test(text || ''); }
...
function quickDiff(before = '', after = '') {
  const a = before.split('\n'); const b = after.split('\n'); const max = Math.max(a.length, b.length); const rows = [];
  for (let i = 0; i < max; i += 1) if (a[i] !== b[i]) rows.push({ line: i + 1, before: a[i] ?? '', after: b[i] ?? '' });
...
export default function App() {
  ...
  useEffect(() => {
    ...
    setWorkspace(next); writeWorkspace(next);
  ...
  async function openWorkspaceProject(id) { await run('Opening workspace project...', async () => { const loaded = await post(`/projects/${id}/validate`); setProject(loaded); const first = Object.keys(loaded.files || {}).sort()[0] || ''; await selectFile(first, loaded); addChat('system', `Opened ${loaded.spec?.app_name || id}.`); }); }
  async function loadFolder(event) { await run('Loading folder into workbench...', async () => { const selected = Array.from(event.target.files || []).filter((file) => isTextFile(file.webkitRelativePath || file.name)); const pairs = await Promise.all(selected.slice(0, 500).map(async (file) => [file.webkitRelativePath || file.name, await file.text()])); const sourceFiles = Object.fromEntries(pairs.map(([path, text]) => [path.split('/').slice(1).join('/') || path, text])); const imported = await post('/projects/import/files', { name: selected[0]?.webkitRelativePath?.split('/')?.[0] || 'Imported Project', files: sourceFiles }); setProject(imported); const first = Object.keys(imported.files || {}).sort()[0] || ''; await selectFile(first, imported); addChat('system', `Imported ${Object.keys(imported.files || {}).length} files. Project Brain is ready.`); }); }
  async function createProjectFromIdea(text = idea) { const created = await post('/projects', { idea: text, use_ai: true }); setProject(created); setSelectedPath(''); setContent(''); setInspection(null); return created; }
  ...
  async function buildAll() { if (!projectId) return; await run('Building and repairing...', async () => setProject(await post(`/projects/${projectId}/build-all`))); }
  async function validate() { if (!projectId) return; await run('Validating and repairing...', async () => setProject(await post(`/projects/${projectId}/validate`))); }
  async function runSandbox(id = projectId) { if (!id) return null; const result = await post(`/projects/${id}/sandbox/start`); setSandbox(result); await refreshSandbox(id); return result; }
  async function workErrors() { if (!projectId) return; await run('Working through errors...', async () => { let result = await runSandbox(); for (let round = 1; round <= 8; round += 1) { if (result?.build_ok) break; setNotice(`Repair round ${round}...`); result = await post(`/projects/${projectId}/sandbox/fix`, { instruction }); } setSandbox(result); await refreshSandbox(); addChat('codem8s', result?.build_ok ? 'Build is green and preview is live.' : 'I worked through repair rounds but it still needs attention.'); }); }
  async function autonomousMode(text = command || instruction) { if (!projectId) return; await run('Autonomous mode: working until it runs...', async () => { addChat('you', text || 'Work until it runs.'); const result = await post(`/projects/${projectId}/autonomous`, { instruction: text || 'Work through the project until it runs.', max_rounds: 12 }); if (result.project) setProject(result.project); if (result.sandbox) setSandbox(result.sandbox); await refreshSandbox(projectId); addChat('codem8s', result.project?.status === 'valid' ? 'Autonomous mode finished: project is valid and preview should be live.' : 'Autonomous mode reached a stopping point. Check logs or give me a narrower instruction.'); }); }

  async function saveFile(runAfter = selectedPath.endsWith('.html')) { if (!projectId || !selectedPath) return; await run(`Saving ${selectedPath}...`, async () => { const saved = await post(`/projects/${projectId}/files/save`, { path: selectedPath, content, instruction: 'Manual workbench edit' }); setProject(saved); await selectFile(selectedPath, saved); if (runAfter) await runSandbox(saved.project_id); addChat('system', runAfter ? `Saved and previewed ${selectedPath}.` : `Saved ${selectedPath}.`); }); }
  async function proposeEditFile(text = editInstruction) { if (!projectId || !selectedPath) return null; const original = content; const result = await post(`/projects/${projectId}/files/refactor`, { path: selectedPath, content, instruction: text }); const updated = result.file.content || ''; setPendingEdit({ path: selectedPath, before: original, after: updated, project: result.project, inspection: result.inspection, diff: quickDiff(original, updated), instruction: text }); setContent(original); addChat('codem8s', `Prepared changes for ${selectedPath}. Review the diff and approve or reject.`); return result; }
  async function approvePendingEdit(runAfter = selectedPath.endsWith('.html')) { if (!pendingEdit) return; setProject(pendingEdit.project); setContent(pendingEdit.after); setInspection(pendingEdit.inspection || null); addChat('system', `Approved AI change to ${pendingEdit.path}.`); const id = pendingEdit.project.project_id; setPendingEdit(null); if (runAfter) await runSandbox(id); }
  function rejectPendingEdit() { if (!pendingEdit) return; setContent(pendingEdit.before); addChat('system', `Rejected AI change to ${pendingEdit.path}.`); setPendingEdit(null); }

  async function runUnifiedCommand() {
    const text = command.trim(); if (!text) return; setCommand(''); addChat('you', text);
    await run('Codem8s is working...', async () => {
      let activeProject = project;
      if (!activeProject) { activeProject = await createProjectFromIdea(text); addChat('codem8s', 'Created a new project from your request. Ask me to build/run it, or press Autonomous: Do Everything.'); return; }
      if (/\b(keep working|autonomous|do everything|finish it|until it runs|make it work)\b/i.test(text)) { const result = await post(`/projects/${activeProject.project_id}/autonomous`, { instruction: text, max_rounds: 12 }); if (result.project) setProject(result.project); if (result.sandbox) setSandbox(result.sandbox); await refreshSandbox(activeProject.project_id); addChat('codem8s', 'Ran autonomous mode. I generated, repaired, built, and tried to preview the project.'); return; }
      if (/\b(explain|understand|architecture|brain|map|overview)\b/i.test(text) && brain) { addChat('codem8s', `Project Brain: ${brain.summary}\nEntry points: ${brain.entryPoints.join(', ') || 'none'}\nRisks: ${brain.risks.join('; ') || 'none'}`); return; }
      if (wantsWholeProject(text)) { const changed = await post(`/projects/${activeProject.project_id}/change`, { instruction: text }); setProject(changed); activeProject = changed; addChat('codem8s', 'Applied this as a project-wide instruction. Use Autonomous to execute it fully.'); }
      else if (selectedPath) { await proposeEditFile(text); }
      else { const result = await post(`/projects/${activeProject.project_id}/team/run`, { goal: text, max_cycles: 1 }); if (result.project) { setProject(result.project); activeProject = result.project; } addChat('codem8s', 'Ran the project agents on your request.'); }
      if (wantsRun(text) && !selectedPath) { const result = await post(`/projects/${activeProject.project_id}/sandbox/start`); setSandbox(result); await refreshSandbox(activeProject.project_id); addChat('codem8s', result.build_ok ? 'Preview is live.' : 'Build/preview is not green yet. Press Autonomous or Make It Work.'); }
    });
  }
  ...
  return (
    <main className="app">
      ...
      {pendingEdit && <section className="card diff-card"><h2>AI Change Preview</h2><p><b>{pendingEdit.path}</b> — {pendingEdit.diff.length} changed line(s) shown. Approve to apply this version, or reject to keep the current code.</p><div className="row"><button onClick={() => approvePendingEdit(false)}>Approve</button><button onClick={() => approvePendingEdit(true)}>Approve + Preview</button><button className="warning" onClick={rejectPendingEdit}>Reject</button></div><div className="diff-list">{pendingEdit.diff.map((row) => <div className="diff-row" key={row.line}><b>{row.line}</b><pre className="diff-before">- {row.before}</pre><pre className="diff-after">+ {row.after}</pre></div>)}</div></section>}
