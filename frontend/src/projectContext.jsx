...
export function ProjectProvider({ children }) {
  ...
  async function buildAllIncremental(id = project?.project_id, onProgress = null) {
    if (!id) return null;
    let current = await request(`/projects/${id}`);
    const MAX_STEPS = 200;
    for (let i = 0; i < MAX_STEPS; i += 1) {
      const pending = Object.values(current.files || {}).filter((f) => f.status !== 'valid' && f.status !== 'needs_repair');
      if (!pending.length) break;
      onProgress?.(i + 1, pending.length, current);
      current = await post(`/projects/${id}/build-next`, {});
      setProject(current);
    }
  ...
  async function buildAll(id = project?.project_id, onProgress = null) {
    if (!id) return null;
    return buildAllIncremental(id, onProgress);
  }
  ...
  async function createProject(idea, options = {}) {
    ...
    const built = await buildAllIncremental(created.project_id, (step, remaining) => {
      setNotice(`Building file ${step} (${remaining} remaining)...`);
    });
    return built;
  }
