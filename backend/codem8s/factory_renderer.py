...
from .factory_planner import app_kind, fields_for, workflows_for
...
def app_jsx(spec: ProjectSpec) -> str:
    ...
    pages_json = json.dumps(workflows_for(spec.goal))
...
export default function App() {{
  ...
  const [activePage, setActivePage] = useState(PAGES[0] || 'Records');
  ...
  return (
    <main className="app">
      ...
      {profile_node}
      <WorkflowTabs pages={{PAGES}} active={{activePage}} onChange={{setActivePage}} />
      {dash_node}
      <WorkflowPage page={{activePage}} fields={{FIELDS}} items={{items}} />
...
export default function DynamicForm({ fields, onSubmit, error }) {
  ...
  return <form className="card" onSubmit={submit}><h2>Create</h2>{fields.map((field) => <label key={field.name}><span>{field.label}</span>{field.type === 'textarea' ? <textarea value={form[field.name] || ''} onChange={(event) => update(field.name, event.target.value)} /> : field.type === 'select' ? <select value={form[field.name] || field.default || ''} onChange={(event) => update(field.name, event.target.value)}>{(field.options || []).map((option) => <option key={option} value={option}>{option}</option>)}</select> : <input type={field.type || 'text'} value={form[field.name] || ''} onChange={(event) => update(field.name, event.target.value)} />}</label>)}<button type="submit">Save</button>{error && <p className="error">{error}</p>}</form>;
}
'''
...
export default function RecordList({ fields, items, onDelete }) {
  ...
  return <section className="card"><h2>Records</h2>{items.length === 0 && <p>No records yet.</p>}{items.map((item) => <article className="item" key={item.id}><div><b>{item[primary]}</b>{secondary && <p>{item[secondary]}</p>}<div className="meta">{fields.slice(2).map((field) => item[field.name] ? <span key={field.name}>{field.label}: {item[field.name]}</span> : null)}</div></div><button onClick={() => onDelete(item.id)}>Delete</button></article>)}</section>;
}
'''


def workflow_tabs_jsx() -> str:
    return '''import React from 'react';

export default function WorkflowTabs({ pages, active, onChange }) {
  return <nav className="tabs">{pages.map((page) => <button className={page === active ? 'active' : ''} key={page} onClick={() => onChange(page)}>{page}</button>)}</nav>;
}
'''


def workflow_page_jsx() -> str:
    return '''import React from 'react';

const PURPOSES = {Leads:'Capture new leads and qualify opportunities.',Pipeline:'Group records by stage so the workflow feels like a board.',Companies:'Review related companies and organisations.',Tasks:'Track follow ups and next actions.',Products:'Manage the product catalogue.',Stock:'Watch stock levels and low stock states.',Suppliers:'Store supplier and ordering notes.',Movements:'Show stock in and stock out activity.',Appointments:'Manage dated bookings and appointments.',Customers:'Review customer records.',Services:'List services that can be booked.',Calendar:'Show a simple schedule overview.',Jobs:'Track job applications.',Interviews:'Focus on interview-stage applications.',FollowUps:'Show records needing a next action.',Board:'Show stages as a simple board.',Priorities:'Group tasks by priority.',Notes:'Browse saved notes.',Categories:'Group records by category.',Archive:'Show archived records.',Expenses:'Track spending and invoices.',Payments:'Review paid and pending payment states.',Records:'Manage records.',Dashboard:'Summary view.'};

export default function WorkflowPage({ page, fields, items }) {
  const statusField = fields.find((field) => field.name === 'status');
  const groups = statusField?.options || [];
  return <section className="card workflow"><h2>{page}</h2><p>{PURPOSES[page] || 'Workflow page generated from your app idea.'}</p>{groups.length > 0 && <div className="columns">{groups.map((group) => <div className="column" key={group}><h3>{group}</h3><p>{items.filter((item) => item.status === group).length} records</p></div>)}</div>}</section>;
}
'''
...
export default function Dashboard({ items, fields }) {
  ...
  return <section className="card dashboard"><h2>Dashboard</h2><div className="chips"><span>Total: {items.length}</span>{Object.entries(counts).map(([key, value]) => <span key={key}>{key}: {value}</span>)}</div></section>;
}
'''
...
export default function ProfilePanel() {
  ...
  function submit(event) { event.preventDefault(); setSaved(email.trim()); }
...
def render_file(path: str, spec: ProjectSpec) -> Optional[str]:
    ...
    if path == "frontend/src/components/WorkflowTabs.jsx": return workflow_tabs_jsx()
    if path == "frontend/src/components/WorkflowPage.jsx": return workflow_page_jsx()
