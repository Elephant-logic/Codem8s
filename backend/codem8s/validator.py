...
BANNED_PHRASES = [
    ...
    "pass #",
    "welcome to your",
    "comprehensive tool for managing",
...
REQUIRED_EXPORTS = {
    ...
    "backend/validator.py": ["validate_file", "validate_project_against_spec", "detect_placeholders"],
...
def detect_placeholders(content: str) -> List[str]:
    ...
    for phrase in BANNED_PHRASES:
        if phrase.lower() in lowered:
            found.append(phrase)
    if re.search(
        r"(?i)(placeholder|empty|fake|todo|temporary)\s+stub|stub\s+(component|file|implementation|function|screen|placeholder)",
        content,
    ):
...
def detect_placeholder_only_file(path: str, content: str) -> List[str]:
    ...
    if path.endswith(ASSET_EXTENSIONS):
        ...
        if path.endswith((".png", ".jpg", ".jpeg", ".gif", ".webp", ".ico")) and not stripped.startswith(
            ("data:image/", "BASE64:", "iVBOR", "/9j/", "R0lGOD")
        ):
    ...
    if len(lines) <= 3 and all(
        re.match(r"^(#|//|/\*|\*)\s*(File|Path)\s*:", line, re.I) or line in {"*/"}
        for line in lines
    ):
...
def detect_recovered_fallback(path: str, content: str) -> List[str]:
    ...
    if not path.endswith(JS_EXTENSIONS):
        return errors
    is_fallback = (
        "data-generated-fallback" in content
        or "Recovered module" in content
        or "AI builder failed or returned weak code" in content
    )
    if is_fallback:
        if path.endswith(CORE_RENDERED_FILES):
    ...
    for path, content in files.items():
        ...
        for kind, names_text, import_path in iter_imports(content):
            ...
            if not target:
                ...
                continue
            if target.endswith(".css") or target not in export_cache:
                continue
            ...
            if kind == "named":
                for raw_name in names_text.split(","):
                    ...
                    if name not in exported:
                        errors.append(
                            f"{path}: imports named export {name} from {target}, but {target} exports {sorted(exported) or 'no named exports'}"
                        )
...
def validate_product_depth(files: Dict[str, str]) -> List[str]:
    ...
    if dashboard:
        low = dashboard.lower()
        dashboard_sections = sum(
            term in low
            for term in ["kpi", "metric", "activity", "status", "chart", "table", "recent", "priority", "progress", "timeline"]
        )
    ...
    if sample:
        low = sample.lower()
        real_data_signals = sum(
            term in low
            for term in ["priority", "status", "date", "comment", "activity", "team", "due", "progress", "owner", "note"]
        )
...
def validate_project_against_spec(files: Dict[str, str], spec_files: Dict[str, str]) -> Tuple[bool, List[str]]:
    ...
    for path in spec_files:
        if path not in files or not str(files.get(path, "")).strip():
            errors.append(f"{path}: missing required planned file")
