...
def local_quality_problems(spec: ProjectSpec, files: dict[str, str]) -> list[dict[str, str]]:
    ...
    if "snake" in goal:
        ...
        combined = "\n".join(files.values()).lower().replace("game over", "gameover")
        missing = [term for term in required_terms if term not in combined]
        if missing:
...
Reject if implementation is much smaller/weaker than blueprint.
Reject if spec promises mechanics/pages/features that code does not implement.
Reject boring CSS and tiny demo apps.
Reject duplicate or disconnected game logic.
Reject fake comments, future-work notes, TODOs, or placeholders.
"""
...
- If CSS, make the design polished, responsive, modern, with layout, cards, gradients/shadows/controls.
"""
    payload = {
        "blueprint": blueprint,
        "target_path": path,
...
def repair_project(spec: ProjectSpec, files: dict[str, str]) -> dict[str, str]:
    ...
    # One more local pass: if CSS is still weak and exists, force a CSS repair.
    if "frontend/src/styles.css" in changed and any(p["path"] == "frontend/src/styles.css" for p in local_quality_problems(spec, changed)):
        repaired = repair_project_file(spec, "frontend/src/styles.css", changed, "CSS still too basic after repair")
        if repaired:
            changed["frontend/src/styles.css"] = repaired
