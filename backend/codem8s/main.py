...
def repair_whole_project(state: BuildState) -> None:
    ...
    try:
        ...
        state.logs.append(f"Whole-project repair skipped: {exc}")
        return
    ...
    state.logs.append(f"Whole-project repair updated {changed} file(s)" if changed else "Whole-project repair checked")
...
def export(project_id: str):
    ...
    # IMPORTANT:
    # Do not call AI repair here. Export must be fast and deterministic.
    # Repair runs during Build All and Validate only.
