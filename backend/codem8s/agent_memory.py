...
STORE_DIR = DEFAULT_DATA_DIR / "agent_memory"
LEGACY_STORE_DIR = Path('/opt/render/.codem8s/agent_memory')
...
def candidate_dirs() -> list[Path]:
    seen: set[str] = set()
    dirs: list[Path] = []
    for candidate in [memory_dir(), STORE_DIR, LEGACY_STORE_DIR, FALLBACK_DIR]:
        key = str(candidate)
...
def list_memory(category: str | None = None, tag: str | None = None, limit: int = 100) -> list[MemoryRecord]:
    ...
    for directory in candidate_dirs():
        for path in directory.glob('*.json'):
            record = _load(path)
            if not record or record.memory_id in seen_ids:
                continue
...
def seed_core_memories() -> None:
    if any(directory.glob('*.json') for directory in candidate_dirs()):
        return
    seeds = [
